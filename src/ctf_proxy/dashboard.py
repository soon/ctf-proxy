...
class ServiceOverviewWidget(Static):
    ...
    def compose(self) -> ComposeResult:
        with Container():
            yield Label(f"[bold]{self.service_name}[/bold] ({self.service_type.upper()})", classes="service-title")
            yield Label(f"Port: {self.port}")
            
            with Horizontal():
                with Vertical():
                    yield Label("[bold]Total Requests[/bold]")
                    yield Label(str(self.metrics.get_request_count()), id=f"requests-{self.service_name}")
                
                with Vertical():
                    yield Label("[bold]Error Rate[/bold]")
                    error_rate = self.metrics.get_error_rate()
                    color = "red" if error_rate > 5 else "yellow" if error_rate > 1 else "green"
                    yield Label(f"[{color}]{error_rate:.1f}%[/{color}]", id=f"error-rate-{self.service_name}")
...
class CTFProxyDashboard(App):
    ...
    def __init__(self, config_path: str, db_path: str):
        ...
        self.metrics = {
            service.name: ServiceMetrics(service.name, self.db) 
