...
from ctf_proxy.analyzer.db import AnalysisDB, make_analysis_db
from ctf_proxy.analyzer.preview import HTTP, TCP, PreviewRunner
from ctf_proxy.analyzer.rules_store import DRAFT, ENABLED, RulesStore, RuleValidationError
...
DEFAULT_SOURCE_DB_FILE = "/app/data/proxy_stats.db"
DEFAULT_ANALYSIS_DB_FILE = "/app/data/analysis.db"
DEFAULT_RULES_FOLDER = "/app/rules"
...
def init_app(
    source_db_file: str = DEFAULT_SOURCE_DB_FILE,
    rules_folder: str = DEFAULT_RULES_FOLDER,
    analysis_db_file: str = DEFAULT_ANALYSIS_DB_FILE,
) -> None:
    global store, preview_runner, analysis_db, source_reader
    store = RulesStore(rules_folder)
    preview_runner = PreviewRunner(source_db_file)
    analysis_db = make_analysis_db(analysis_db_file)
    source_reader = SourceReader(source_db_file)


@asynccontextmanager
async def lifespan(app: FastAPI):
    source_db_file = os.environ.get("SOURCE_DB_FILE", DEFAULT_SOURCE_DB_FILE)
    rules_folder = os.environ.get("RULES_FOLDER", DEFAULT_RULES_FOLDER)
    analysis_db_file = os.environ.get("ANALYSIS_DB_FILE", DEFAULT_ANALYSIS_DB_FILE)
    init_app(source_db_file, rules_folder, analysis_db_file)
    yield
