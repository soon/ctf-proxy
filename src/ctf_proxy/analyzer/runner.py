...
from ctf_proxy.analyzer.db import AnalysisResultRow, make_analysis_db
...
class AnalyzerRunner:
    def __init__(
        ...
        self.db = make_analysis_db(analysis_db_file)
        self.registry = RuleRegistry(rules_folder)
    ...
    def run(self) -> None:
        ...
        logger.info(f"Source database: {self.source.db_file}")
        logger.info(f"Analysis database: {self.db.db_file}")
        logger.info(f"Rules folder: {self.registry.rules_folder}")
...
def main():
    ...
    runner = AnalyzerRunner(source_db_file, analysis_db_file, rules_folder, batch_size)
    runner.run()
