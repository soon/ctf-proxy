...
import { getServiceQueryStatsApiServicesPortQueriesGetOptions } from "@/client/@tanstack/react-query.gen";
import {
	Table,
	Tag,
	Button,
	...
	return (
		<div style={{ width: "280px", height: 50, padding: "5px 0" }}>
			<svg
				...
				aria-label="Query activity sparkline chart"
			>
				<title>Query activity sparkline chart</title>
				<line
					x1={padding}
					y1={height - padding}
					x2={width - padding}
...
function QueryStats() {
	...
	const { data, isLoading, error, refetch, isFetching } = useQuery({
		...getServiceQueryStatsApiServicesPortQueriesGetOptions(queryParams),
	...
	if (isLoading) {
		return (
			<div className="flex justify-center items-center h-64">
				<Spin size="large" tip="Loading query statistics..." />
			</div>
		);
	}
	...
	if (error || !data) {
		return (
			<div className="flex justify-center items-center h-64">
				<Empty description={error?.message || "Query stats not available"} />
			</div>
		);
	}
	...
	const columns: ColumnsType<QueryRow> = [
		...
		},
		{
			...
			key: "total_count",
			width: 120,
			sorter: (a: QueryRow, b: QueryRow) => b.total_count - a.total_count,
	...
	return (
		<div className="space-y-4">
			...
			{ignored_queries && ignored_queries.length > 0 && (
				<Alert
					message="Ignored Query Parameters"
					description={ignored_queries.join(", ")}
					type="info"
					showIcon
					closable
				/>
			...
			{queries.length > 0 ? (
				<Table
					columns={columns}
					dataSource={queries}
					rowKey={(record) => `${record.param}-${record.value}`}
					size="small"
					pagination={{
						pageSize: 20,
						...
						showTotal: (total, range) => (
							<span className="text-xs">
								{range[0]}-{range[1]} of {total} query parameters
							</span>
						),
					}}
					scroll={{ x: 600 }}
				...
				<Empty description="No query data available" />
			)}
