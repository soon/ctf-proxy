...
class BaseTimeStats:
    ...
    def get_time_series_with_totals(
        ...
        with self.db.connect() as conn:
            ...
            rows = self.queries.time_series_with_totals_rows(
                cursor, self.table_name, self.key_columns, port, window_start_truncated
            )
    ...
    def get_time_series_for_range(
        ...
        with self.db.connect() as conn:
            ...
            rows = self.queries.time_series_range_rows(
                cursor, self.table_name, self.key_columns, port, start_ms, end_ms
            )
