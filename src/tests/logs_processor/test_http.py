...
from ctf_proxy.logs_processor.http import HttpTapProcessor
...
def process_tap(db: ProxyStatsDB, file_path: str, tap_id="test-id", batch_id="test-batch"):
    ...
    config = Config("tests/logs_processor/data/test-config.yaml")
...
def test_ws_tap_processing(db):
    process_tap(db, "tests/logs_processor/data/taps/http/ws.json")
