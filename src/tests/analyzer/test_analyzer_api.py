...
from ctf_proxy.analyzer import api
...
def client(tmp_path):
    ...
    (rules_folder / "preview").mkdir(parents=True)
    (rules_folder / "enabled_one.py").write_text(ENABLED_RULE)

    ids = seed_source_db(source_db_file)
    analysis_db_file = str(tmp_path / "analysis.db")
    api.init_app(source_db_file, str(rules_folder), analysis_db_file)
