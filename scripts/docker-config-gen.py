...
def run_docker_command(args: List[str]) -> str:
    ...
    try:
        ...
        print("Error: Docker not found. Make sure Docker is installed and in PATH.", file=sys.stderr)
        sys.exit(1)
...
def determine_service_type(port: int, protocol: str, container_name: str) -> str:
    ...
    if any(indicator in container_lower for indicator in ['web', 'http', 'api', 'frontend', 'backend']):
        return 'http'
